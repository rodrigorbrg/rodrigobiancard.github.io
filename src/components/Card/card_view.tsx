  ...
  return (
    <div key={tech}>
      <div className="mb-5 p-10 bg-black items-center rounded-3xl transition-all hover:animate-shadow-image duration-200" >
        ...
        <h4 className="font-normal text-2xl mb-4 leading-5 block box-sizing antialiased text-white ">{tech}</h4>
        <div className="h-1 w-12 m-auto mb-2 bg-[#04e5e5] block "></div>
        <ul className="mt-2 p-0">
          {
            descriptions.map(element => {
              return  <li key={element} className="before:gradient-slow before:animate-gradient-slow before:inline-block before:relative before:bottom-1 before:content-[''] before:rounded-full before:h-1 before:w-1 before:mr-2 relative p-0 my-1">{element}</li>
            })
          }
        </ul>
      </div>
